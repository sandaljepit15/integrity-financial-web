<template>
  <div class="daftar-anggaran">
        ...
        <table class="table table-striped table-hover table-bordered align-middle mb-0" style="min-width: 1200px;">
          <thead class="table-dark text-center align-middle">
            <tr>
              ...
              <th width="11%">Harga Total</th>
              <th width="8%">Ket.</th>
              <th width="6%">Aksi</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="anggarans.length === 0">
              <td colspan="11" class="text-center text-muted py-5">
                <i class="bi bi-inbox fs-2 d-block mb-2"></i> Belum ada data anggaran.
              </td>
            </tr>
            
            <tr v-for="(item, index) in anggarans" :key="index" :class="{'bg-light fw-medium': item.status === 'H' || item.status === 'DH'}">
              ...
              <td class="text-center">
                <span class="badge" :class="item.jenis === 'in' ? 'bg-success' : 'bg-danger'">{{ item.jenis?.toUpperCase() }}</span>
              </td>
              <td class="text-center">
                <span class="badge bg-secondary" v-if="item.status === 'H'">Header</span>
                <span class="badge bg-secondary" v-else-if="item.status === 'DH'">Detail Header</span>
                <span class="badge bg-primary" v-else>Detail</span>
              </td>
              ...
              <td class="text-center">
                <div class="text-truncate text-muted d-inline-block" style="max-width: 100px; font-size: 0.85rem;" :title="item.keterangan">
                  {{ item.keterangan || '-' }}
                </div>
              </td>
              <td class="text-center text-nowrap">
                <button class="btn btn-sm btn-light text-primary border me-1 shadow-sm" @click="openEditModal(item)" title="Edit">
                  <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-light text-danger border shadow-sm" @click="deleteRow(item.coa_anggaran, item.uraian)" title="Hapus">
    ...
    <div v-if="isModalOpen" class="custom-modal-overlay">
      <div class="custom-modal-card card border-0 shadow-lg rounded-4 overflow-hidden" :class="isEditMode ? 'modal-wide' : 'modal-normal'">
            ...
            <div class="card-body p-4 max-h-600 overflow-auto custom-scrollbar">
              ...
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <input type="text" class="form-control" v-model="form.coa_anggaran" :disabled="isEditMode" placeholder="Contoh: 1.1">
                  <small v-if="isEditMode" class="text-danger" style="font-size: 0.7rem;">*Kode COA tidak dapat diubah setelah dibuat.</small>
                </div>
                <div class="col-md-6">
                  <label class="form-label small fw-bold">Induk COA (Opsional)</label>
                  <select class="form-select" v-model="form.induk" :disabled="form.status === 'H'">
                ...
                <div class="col-md-12">
                  ...
                  <input type="text" class="form-control" v-model="form.uraian" placeholder="Masukkan nama anggaran...">
                </div>

                <div class="col-md-6">
                  <label class="form-label small fw-bold">Status Akun</label>
                  <select class="form-select" v-model="form.status" @change="recalculateFormTotal">
                ...
                <div class="col-md-4">
                  ...
                  <input type="text" class="form-control" v-model="form.satuan" placeholder="Pcs, Kg, dll" :disabled="form.status !== 'D'">
                </div>
                <div class="col-md-4">
                  <label class="form-label small fw-bold">Volume / Jumlah</label>
                  <input type="text" class="form-control text-end" :value="formatRupiah(form.jumlah)" @input="e => onFormNumberInput('jumlah', (e.target as HTMLInputElement).value)" :disabled="form.status !== 'D'">
                </div>
                <div class="col-md-4">
                  <label class="form-label small fw-bold">Harga Satuan (Rp)</label>
                  <input type="text" class="form-control text-end" :value="formatRupiah(form.harga_satuan)" @input="e => onFormNumberInput('harga_satuan', (e.target as HTMLInputElement).value)" :disabled="form.status !== 'D'">
                </div>

                <div class="col-md-12">
                  <div class="p-3 bg-light border rounded mt-2 d-flex justify-content-between align-items-center">
                ...
                <div class="col-md-12">
                  ...
                  <textarea class="form-control" v-model="form.keterangan" rows="3" placeholder="Tambahkan deskripsi atau catatan..."></textarea>
                </div>
              </div>
