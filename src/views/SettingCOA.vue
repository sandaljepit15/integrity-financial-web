<template>
  <div class="setting-coa">
    ...
    <div class="card border-0 shadow-sm rounded-0 p-3 mb-4 bg-primary bg-opacity-10 border-start border-primary border-4">
      <div class="row align-items-center">
        ...
        <div class="col-md-5 d-flex gap-2">
          <select class="form-select border-primary rounded-0" v-model="companyLabaRugiCoa">
          ...
          <button class="btn btn-primary fw-bold px-4 rounded-0" @click="saveSystemAccounts">
    ...
    <div class="d-flex mb-3 align-items-center">
      <div class="input-group input-group-sm w-25">
        ...
        <input type="text" class="form-control rounded-0 border-start-0 border-secondary ps-0" v-model="searchQuery" placeholder="Cari COA atau Nama Akun...">
      </div>
    </div>
    ...
    <div class="card border border-secondary shadow-sm rounded-0 overflow-hidden">
      <div class="table-responsive" style="min-height: 400px;">
        <table class="table table-sm table-striped table-hover table-bordered align-middle mb-0" style="min-width: 900px; font-size: 0.85rem;">
          <thead class="table-dark text-center align-middle">
            <tr>
              <th width="5%" class="py-2">No</th>
              <th width="15%" class="py-2">COA</th>
              <th width="35%" class="py-2">Nama Akun</th>
              <th width="10%" class="py-2">Sifat</th>
              <th width="20%" class="py-2">Jenis / Kelompok</th>
              <th width="15%" class="py-2">Aksi</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr v-for="(item, index) in filteredCOAs" :key="item.coa_code">
              ...
              <td class="text-center">
                <span class="badge rounded-0" :class="item.sifat === 'H' ? 'bg-dark' : (item.sifat === 'DH' ? 'bg-secondary' : 'bg-primary')">
                  {{ item.sifat }}
                </span>
              </td>
              ...
              <td class="text-center text-nowrap">
                <button class="btn btn-sm btn-light text-primary border me-1 rounded-0 shadow-sm" @click="openEditModal(item)" title="Edit COA">
                  <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-light text-danger border rounded-0 shadow-sm" @click="deleteCOA(item.coa_code, item.nama)" title="Hapus COA">
    ...
    <div v-if="isModalOpen" class="custom-modal-overlay">
      <div class="custom-modal-card card border-0 shadow-lg rounded-0 overflow-hidden">
        ...
        <div class="card-body p-4 max-h-500 overflow-auto custom-scrollbar">
          <div class="row g-3">
            <div class="col-12">
              ...
              <select v-model="form.kelompok" class="form-select rounded-0">
            ...
            <div class="col-12">
              ...
              <input type="text" class="form-control rounded-0" v-model="form.coa_code" :disabled="isEditMode" placeholder="Contoh: 1.1">
            </div>

            <div class="col-12">
              <label class="form-label small fw-bold">Nama Akun <span class="text-danger">*</span></label>
              <input type="text" class="form-control rounded-0" v-model="form.nama" placeholder="Contoh: Kas Kecil">
            </div>

            <div class="col-12">
              <label class="form-label small fw-bold">Sifat Akun</label>
              <select class="form-select rounded-0" v-model="form.sifat">
        ...
        <div class="card-footer bg-light border-top text-end py-3 px-4">
          ...
          <button class="btn btn-primary fw-bold px-4 rounded-0" @click="saveCOA">
...
const coas = ref<any[]>([])
const companyLabaRugiCoa = ref('') 

const isModalOpen = ref(false)
const isEditMode = ref(false)
const searchQuery = ref('') // STATE PENCARIAN BARU

const form = ref({ coa_code: '', nama: '', sifat: 'D', kelompok: '1' })
...
const filteredCOAs = computed(() => {
  if (!searchQuery.value) return coas.value
  const q = searchQuery.value.toLowerCase()
  return coas.value.filter(c => 
...
const fetchSystemAccounts = async () => {
  try {
    ...
    if (data) companyLabaRugiCoa.value = data.coa_laba_rugi_berjalan || ''
...
const saveSystemAccounts = async () => {
  ...
  try {
    const { error } = await supabase.from('company_profile').update({ coa_laba_rugi_berjalan: companyLabaRugiCoa.value }).eq('id', 1) 
...
const fetchCOA = async () => {
  ...
  try {
    ...
    coas.value = data || []
    AppAlert.close()
  } catch (err) { AppAlert.error('Gagal memuat', err) }
}
...
const formatKelompok = (kode: string) => {
  const map: Record<string, string> = { '1': '1 - Aset', '2': '2 - Kewajiban', '3': '3 - Ekuitas', '4': '4 - Pendapatan', '5': '5 - Beban' }
