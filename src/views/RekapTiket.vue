    ...
    <div class="screen-only">
      ...
      <div class="d-flex mb-3 gap-2 align-items-center">
        <select class="form-select form-select-sm w-auto rounded-0 border-secondary" v-model="searchColumn">
          ...
          <option value="nama_platform">Platform Tiket</option>
          ...
          <table class="table table-sm table-striped table-hover table-bordered align-middle mb-0" style="min-width: 1200px; font-size: 0.85rem;">
            <thead class="table-dark text-center align-middle">
              <tr>
                ...
                <th width="15%" class="py-3 cursor-pointer" @click="handleSort('no_bukti_internal')">
                ...
                <th width="12%" class="py-3 cursor-pointer" @click="handleSort('tanggal_rekap')">
                  Tgl Rekap <i class="bi ms-1" :class="sortColumn==='tanggal_rekap' ? (sortDirection==='asc' ? 'bi-sort-numeric-down' : 'bi-sort-numeric-up') : 'bi-arrow-down-up opacity-25'"></i>
                </th>
                <th width="16%" class="py-3 cursor-pointer" @click="handleSort('nama_platform')">
                ...
                <th width="12%" class="py-3 cursor-pointer" @click="handleSort('mutasi_kotor')">
                  Mutasi Kotor <i class="bi ms-1" :class="sortColumn==='mutasi_kotor' ? (sortDirection==='asc' ? 'bi-sort-numeric-down' : 'bi-sort-numeric-up') : 'bi-arrow-down-up opacity-25'"></i>
                </th>
            ...
            <tbody>
              ...
              <tr v-for="(item, index) in paginatedData" :key="item.id">
                ...
                <td class="text-primary">{{ item.nama_platform }}</td>
                <td class="fw-bold text-dark">{{ item.nama_eo }}</td>
      ...
      <div class="print-container">
        ...
        <template v-if="printMode === 'jurnal'">
          <table class="w-100 mb-2 info-table text-dark" style="font-size: 9pt;">
            <tr>
              <td width="15%" class="fw-bold p-1 px-2">No Bukti TKR</td>
        ...
        <!-- LAYOUT 2: BUKTI REKAP TIKET (TKR) TERCETAK HISTORIS -->
        <template v-if="printMode === 'rekap'">
          <table class="w-100 mb-4 text-dark" style="font-size: 9pt;">
            ...
            <tr>
              <td class="fw-bold p-1">Pihak Event/EO</td>
              <td class="p-1 fw-bold pe-2 text-uppercase text-danger" style="text-align: right;">{{ itemToPrint.eo_nama }}</td>
            ...
            <tr>
              ...
              <td class="p-1 pe-2" style="text-align: right;">{{ itemToPrint.rekap.created_by }}</td>
            </tr>
          </table>
          ...
          <div class="text-dark" style="font-size: 10pt; border-top: 1px solid #dee2e6; padding-top: 15px;">
            ...
            <table class="w-100 table-print border-dark text-dark mt-3 mb-3">
              <thead class="bg-light fw-bold">
                <tr><th class="p-2 text-center" colspan="2">HISTORIS SALDO & PENYELESAIAN TIKET</th></tr>
              </thead>
              <tbody>
                <tr>
                  <td class="p-2 w-50">Posisi Saldo Tercatat Sebelumnya</td>
                  <td class="p-2 w-50 text-end">Rp {{ formatNominal(itemToPrint.rekap.saldo_tercatat) }}</td>
                </tr>
                <tr>
                  <td class="p-2 fw-bold">Posisi Saldo Baru (Dashboard Saat Ini)</td>
                  <td class="p-2 text-end fw-bold">Rp {{ formatNominal(itemToPrint.rekap.saldo_baru) }}</td>
                </tr>
                <tr class="bg-light">
                  <td class="p-2 fw-bold fst-italic ps-4 text-primary">-> NILAI MUTASI TIKET KOTOR (PIUTANG)</td>
                  <td class="p-2 text-end fw-bold fst-italic text-primary">Rp {{ formatNominal(itemToPrint.rekap.mutasi_kotor) }}</td>
                </tr>
                
                <tr><td colspan="2" class="p-0 border-0" style="height: 10px;"></td></tr>
                <tr><td colspan="2" class="p-2 fw-bold bg-light">Rincian Penyesuaian (Fee, Admin, Pajak, Subsidi):</td></tr>
                
                <tr v-for="(row, idx) in itemToPrint.adjustments" :key="idx">
                  <td class="p-2"><span class="ps-3">{{ row.dampak === 'pengurang' ? '-' : '+' }} {{ row.keterangan }}</span></td>
                ...
                <tr v-if="itemToPrint.adjustments.length === 0">
                  <td colspan="2" class="p-2 text-center text-muted fst-italic">Tidak ada penyesuaian.</td>
                </tr>
              </tbody>
              <tfoot class="fw-bold" style="background-color: #e2e8f0;">
                <tr>
                  <td class="p-2 text-end text-uppercase">TOTAL BERSIH HAK EVENT / HUTANG EO (NETTO) :</td>
            ...
            <p class="fst-italic fw-bold mb-4" style="font-size: 10pt;">
              Terbilang Hak EO : {{ formatTerbilang(itemToPrint.rekap.netto_hutang) }}
            </p>
        ...
        <div class="card-body p-4 modal-body-scroll custom-scrollbar bg-light bg-opacity-50">
          ...
          <div class="bg-white p-3 rounded-0 border shadow-sm mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
              <h6 class="fw-bold mb-0 text-dark">1. Relasi Entitas & Anchor Saldo</h6>
              <span class="badge bg-secondary rounded-0">{{ form.no_bukti_internal }}</span>
            </div>
            
            <div class="row g-3">
              <!-- ANCHOR: MASTER HUTANG (EO) -->
              <div class="col-md-6 border-end">
                <label class="form-label small fw-bold text-danger">Master Hutang (Event / EO) <i class="bi bi-info-circle ms-1" title="Menentukan perhitungan Saldo Tercatat"></i> <span class="text-danger">*</span></label>
                <select class="form-select border-danger rounded-0 mb-2" v-model="form.master_hutang_id" @change="fetchSaldoTercatat">
                  <option value="" disabled>-- Pilih Master Hutang --</option>
                  <option v-for="mh in masterHutangs" :key="mh.id" :value="mh.id">{{ mh.nomor_perjanjian }} - {{ mh.pihak_lawan?.nama }}</option>
                </select>

                <label class="form-label small fw-bold text-muted mt-2">Akun Hutang Utama (Otomatis KREDIT untuk Mutasi Kotor)</label>
                <div class="position-relative dropdown-container">
                  <div class="form-control form-control-sm bg-white d-flex justify-content-between align-items-center cursor-pointer rounded-0" @click="toggleDropdown('coa_hutang')">
                    <span class="text-truncate" :class="{'text-muted': !form.coa_hutang}">{{ getCoaLabel(form.coa_hutang) || 'Pilih COA Hutang...' }}</span>
              ...
              <!-- PIHAK PIUTANG (PLATFORM) -->
              <div class="col-md-6">
                <label class="form-label small fw-bold text-primary">Master Piutang (Platform Tiket) <span class="text-danger">*</span></label>
                <select class="form-select border-primary rounded-0 mb-2" v-model="form.master_piutang_id">
                  <option value="" disabled>-- Pilih Master Piutang --</option>
                  <option v-for="mp in masterPiutangs" :key="mp.id" :value="mp.id">{{ mp.nomor_perjanjian }} - {{ mp.pihak_lawan?.nama }}</option>
                </select>
                
                <label class="form-label small fw-bold text-muted mt-2">Akun Piutang Utama (Otomatis DEBET untuk Mutasi Kotor)</label>
                <div class="position-relative dropdown-container">
                  <div class="form-control form-control-sm bg-white d-flex justify-content-between align-items-center cursor-pointer rounded-0" @click="toggleDropdown('coa_piutang')">
                    <span class="text-truncate" :class="{'text-muted': !form.coa_piutang}">{{ getCoaLabel(form.coa_piutang) || 'Pilih COA Piutang...' }}</span>
                    <i class="bi bi-search small text-muted"></i>
                  </div>
          ...
          <div class="bg-white p-3 rounded-0 border shadow-sm mb-4">
            <h6 class="fw-bold mb-3 border-bottom pb-2">2. Kalkulasi Nilai Mutasi Kotor</h6>
            <div class="row g-3 align-items-end">
              <div class="col-md-4">
                <label class="form-label small fw-bold text-muted">Saldo Tercatat (Histori Event) <i class="bi bi-clock-history ms-1"></i></label>
              ...
              <div class="col-md-4">
                <label class="form-label small fw-bold text-dark">Saldo Saat Ini (Dashboard Tiket) <span class="text-danger">*</span></label>
                <div class="input-group">
              ...
              <div class="col-md-4">
                ...
                <div class="input-group">
                  ...
                  <input type="text" class="form-control bg-primary bg-opacity-10 border-primary fw-bold text-primary text-end rounded-0" :value="formatInputRupiah(kalkulasiMutasi)" readonly>
                </div>
                <small v-if="kalkulasiMutasi < 0" class="text-danger d-block mt-1" style="font-size: 0.7rem;">Nilai baru tidak boleh lebih kecil dari saldo tercatat.</small>
              </div>
              ...
              <div class="col-md-7 mt-4">
                <label class="form-label small fw-bold">Uraian / Keterangan Rekap <span class="text-danger">*</span></label>
                <input type="text" class="form-control rounded-0" v-model="form.keterangan" placeholder="Contoh: Rekap Penjualan Tiket Day 1...">
              </div>
            </div>
          </div>

          <div class="bg-white p-3 rounded-0 border shadow-sm position-relative" style="z-index: 10;">
            <div class="d-flex justify-content-between align-items-center mb-2 border-bottom pb-2">
              <div>
                <h6 class="fw-bold mb-0">3. Jurnal Penyesuaian thd Hak EO (Double-Entry Generator)</h6>
                <small class="text-muted d-block mt-1" style="font-size: 0.75rem;">Setiap baris akan mencatat Jurnal Debet & Kredit secara seimbang. Otomatis mengubah Netto Hak Payout ke EO.</small>
              </div>
            ...
            <div class="table-responsive border rounded-0 mt-3 mb-3 table-visible-overflow position-relative">
              <table class="table table-sm table-bordered align-middle mb-0">
                <thead class="bg-light text-center small">
                  <tr>
                    <th width="20%" class="align-middle">Keterangan / Nama <span class="text-danger">*</span></th>
                    <th width="15%" class="align-middle">Dampak ke Hak EO <span class="text-danger">*</span></th>
                    <th width="22%" class="align-middle">COA DEBET <span class="text-danger">*</span></th>
                    <th width="22%" class="align-middle">COA KREDIT <span class="text-danger">*</span></th>
                    <th width="15%" class="align-middle">Nilai <span class="text-danger">*</span></th>
                    <th width="6%" class="align-middle">Aksi</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(row, idx) in form.penyesuaian" :key="idx">
                    <td>
                      <input type="text" class="form-control form-control-sm rounded-0" v-model="row.keterangan" placeholder="Cth: Estimasi Fee">
                    </td>
                    <td>
                      <!-- PEMBAIKAN 1: BUNGKUS idx dengan Number() -->
                      <select class="form-select form-select-sm rounded-0 text-center" v-model="row.dampak" :class="row.dampak === 'penambah' ? 'text-success fw-bold' : 'text-danger fw-bold'" @change="updateNilaiCalculated(Number(idx))">
                        <option value="pengurang">Pengurang Hak (-)</option>
                        <option value="penambah">Penambah Hak (+)</option>
                      </select>
                    </td>
                    <!-- DROPDOWN COA DEBET -->
                    <td>
                      <div class="position-relative dropdown-container" :style="{ zIndex: activeDropdown === `row-${idx}-debet` ? 1050 : 1 }">
                        <div class="form-control form-control-sm bg-white d-flex justify-content-between align-items-center cursor-pointer rounded-0" @click="toggleDropdown(`row-${idx}-debet`)">
                          <span class="text-truncate d-inline-block" style="max-width: 140px;" :class="{'text-muted': !row.coa_debet}">{{ getCoaLabel(row.coa_debet) }}</span>
                          <i class="bi bi-search small text-muted"></i>
                        </div>
                        <div v-if="activeDropdown === `row-${idx}-debet`" class="position-absolute bg-white border rounded-0 shadow mt-1 p-2 custom-dropdown-menu" style="min-width: 300px;">
                          ...
                          <div class="list-group list-group-flush">
                            <!-- PEMBAIKAN 2: BUNGKUS idx dengan Number() -->
                    ...
                    <td>
                      <div class="position-relative dropdown-container" :style="{ zIndex: activeDropdown === `row-${idx}-kredit` ? 1050 : 1 }">
                        <div class="form-control form-control-sm bg-white d-flex justify-content-between align-items-center cursor-pointer rounded-0" @click="toggleDropdown(`row-${idx}-kredit`)">
                          <span class="text-truncate d-inline-block" style="max-width: 140px;" :class="{'text-muted': !row.coa_kredit}">{{ getCoaLabel(row.coa_kredit) }}</span>
                          <i class="bi bi-search small text-muted"></i>
                        </div>
                        <div v-if="activeDropdown === `row-${idx}-kredit`" class="position-absolute bg-white border rounded-0 shadow mt-1 p-2 custom-dropdown-menu" style="min-width: 300px;">
                          ...
                          <div class="list-group list-group-flush">
                            <!-- PEMBAIKAN 3: BUNGKUS idx dengan Number() -->
                    ...
                    <td>
                      <div class="input-group input-group-sm">
                        <!-- PEMBAIKAN 4: BUNGKUS idx dengan Number() -->
                        <select class="form-select rounded-0 text-center" style="max-width: 55px;" v-model="row.tipe_nilai" @change="updateNilaiCalculated(Number(idx))">
                          <option value="nominal">Rp</option>
                          <option value="persen">%</option>
                        </select>
                        <!-- PEMBAIKAN 5: BUNGKUS idx dengan Number() -->
                        <input type="text" class="form-control text-end fw-bold rounded-0" 
                      ...
                      <div class="text-end mt-1 fw-bold" style="font-size: 0.7rem;" :class="row.dampak === 'pengurang' ? 'text-danger' : 'text-success'">
                        = Rp {{ formatNominal(row.nominal_calculated) }}
                      </div>
                    </td>
                    <td class="text-center align-middle">
                      <!-- PEMBAIKAN 6: BUNGKUS idx dengan Number() -->
                  ...
                  <tr v-if="form.penyesuaian.length === 0">
                    <td colspan="6" class="text-center text-muted fst-italic py-3" style="font-size: 0.8rem;">Tidak ada jurnal penyesuaian (Hak Bersih EO akan sama dengan Mutasi Kotor).</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <!-- SUMMARY PREVIEW BOX -->
          <div class="mt-4 p-3 bg-dark text-white rounded-0 shadow-sm border border-secondary" v-if="kalkulasiMutasi > 0">
            ...
            <div class="row text-center fw-bold" style="font-size: 0.9rem;">
              <div class="col-4 border-end border-secondary">
                ...
                <div class="text-info fs-5">Rp {{ formatNominal(kalkulasiMutasi) }}</div>
              </div>
              <div class="col-4 border-end border-secondary">
                <div class="small opacity-75 fw-normal mb-1">2. Total Jurnal Penyesuaian</div>
                <div class="text-light fs-6 mt-1">
                  <span class="text-success"><i class="bi bi-plus-circle"></i> Hak: Rp {{ formatNominal(totalPenambah) }}</span> | 
                  <span class="text-danger"><i class="bi bi-dash-circle"></i> Hak: Rp {{ formatNominal(totalPengurang) }}</span>
                </div>
              </div>
              <div class="col-4">
                ...
                <div v-if="nilaiHutang < 0" class="small text-danger fw-normal" style="font-size: 0.7rem;">Nilai Netto minus! Periksa kembali persentase/nominal pengurang.</div>
              </div>
            </div>
          </div>
...
const nilaiHutang = computed(() => kalkulasiMutasi.value + totalPenambah.value - totalPengurang.value)
...
const isFormValid = computed(() => {
  ...
  return form.value.master_piutang_id && form.value.coa_piutang && 
         form.value.master_hutang_id && form.value.coa_hutang &&
         form.value.tanggal_rekap && form.value.keterangan && 
         kalkulasiMutasi.value > 0 && nilaiHutang.value >= 0 &&
         !hasEmptyGrid
})
...
const handleGridNilaiInput = (idx: number, e: Event) => {
  ...
  if (target) {
    ...
    if (form.value.penyesuaian[idx].tipe_nilai === 'persen' && val > 100) val = 100
    form.value.penyesuaian[idx].nilai_input = val
...
const updateNilaiCalculated = (idx: number) => {
  ...
  const mutasi = kalkulasiMutasi.value
  let calculated = 0
  if (row.tipe_nilai === 'persen') {
    calculated = Math.floor(mutasi * (Number(row.nilai_input) / 100))
  } else {
    calculated = Number(row.nilai_input)
  }
  form.value.penyesuaian[idx].nominal_calculated = calculated
}
...
const selectGridCoa = (idx: number, field: string, code: string) => { form.value.penyesuaian[idx][field] = code; closeAllDropdowns() }

const addJurnalRow = () => { 
  form.value.penyesuaian.push({ 
    keterangan: '', dampak: 'pengurang', coa_debet: '', coa_kredit: '', 
    tipe_nilai: 'nominal', nilai_input: 0, nominal_calculated: 0 
  }) 
}
const removeJurnalRow = (idx: number) => { form.value.penyesuaian.splice(idx, 1) }

const fetchSaldoTercatat = async () => {
  if (!form.value.master_hutang_id) return
  try {
    const { data: rekapHist, error: err1 } = await supabase.from('rekap_tiket')
      .select('mutasi_kotor')
      .eq('master_hutang_id', form.value.master_hutang_id)
    
    if (err1) throw err1
    if (!rekapHist || rekapHist.length === 0) { form.value.saldo_tercatat = 0; return }

    form.value.saldo_tercatat = rekapHist.reduce((acc, curr) => acc + Number(curr.mutasi_kotor), 0)
  } catch (error) {
    console.error('Error fetch saldo:', error); form.value.saldo_tercatat = 0
  }
}
...
const openAddModal = () => {
  ...
  form.value = { 
    ...
    master_piutang_id: '', coa_piutang: '', master_hutang_id: '', coa_hutang: '',
    saldo_tercatat: 0, saldo_baru: 0, tanggal_rekap: new Date().toISOString().slice(0,10), keterangan: '', 
    penyesuaian: []
...
const printJurnal = async (no_bukti: string) => {
  ...
  try {
    ...
    itemToPrint.value = { header: trxData[0], details: mappedTrx, totalDebet: totalD, totalKredit: totalK }
...
const printRekap = async (no_bukti: string) => {
  ...
  try {
    const { data: rekapData, error: err1 } = await supabase.from('rekap_tiket')
      .select('*, master_piutang(pihak_lawan(nama)), master_hutang(pihak_lawan(nama))')
...
const saveData = async () => {
  if (!isFormValid.value) {
    AppAlert.error('Gagal', 'Pastikan semua kolom relasi wajib diisi, rincian jurnal lengkap, dan mutasi kotor > 0!'); 
    return 
  }
  ...
  try {
    ...
    const payloadRekap = {
      ...
      detail_penyesuaian: form.value.penyesuaian, // Snapshot Abadi
      keterangan: form.value.keterangan,
      created_by: currentUsername,
      created_at: now.toISOString()
    }

    // --- 1. INSERT TAGIHAN PIUTANG (Platform - GROSS) ---
    const payloadTagihanPiutang = {
    ...
    // --- 2. INSERT TAGIHAN HUTANG (EO - GROSS) ---
    // Hutang dicatat KOTOR untuk memastikan sinkronisasi Ledger Hutang yang akurat. Netto disajikan saat Payout (Settlement).
    const payloadTagihanHutang = {
    ...
    // --- 3. INSERT JURNAL BUKU BESAR ---
    const payloadJurnal = []
    
    // Baris 1: Piutang Utama (DEBET) = Mutasi Kotor
    payloadJurnal.push({
    ...
    // Baris 2: Hutang EO (KREDIT) = Mutasi Kotor
    payloadJurnal.push({
      tanggal_transaksi: form.value.tanggal_rekap, kwitansi_internal: internalNoBukti, kwitansi_eksternal: internalNoBukti, 
    ...
    // Baris 3..N: Jurnal Penyesuaian Bebas (Debet & Kredit di-Input Langsung)
    ...
    AppAlert.success('Sukses!', 'Rekap Tiket (TKR) berhasil diamankan. Tagihan Piutang, Hutang Gross, dan Jurnal Ganda otomatis terbentuk.')
