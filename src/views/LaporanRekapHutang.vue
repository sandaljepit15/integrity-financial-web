  ...
  <div class="laporan-rekap-hutang">
    <div class="screen-only">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h4 class="fw-bold text-dark mb-0">Laporan Rekapitulasi Hutang</h4>
          <p class="text-muted small mb-0">Ringkasan saldo awal, penambahan, dan pelunasan hutang per pihak/klien.</p>
        </div>
        <div>
      ...
      <div class="d-flex mb-3 align-items-center" v-if="reportData.length > 0">
        <div class="input-group input-group-sm w-25">
          ...
          <input type="text" class="form-control rounded-0 border-start-0 border-secondary ps-0" v-model="searchTableQuery" placeholder="Cari nama pihak hutang...">
        </div>
      </div>
      ...
      <div class="card border border-secondary shadow-sm rounded-0 overflow-hidden mb-3">
        <div class="card-header bg-dark text-white p-2 px-3 d-flex justify-content-between align-items-center rounded-0">
          <div class="fw-bold" style="font-size: 0.9rem;"><i class="bi bi-journal-check me-2"></i>Rekapitulasi Saldo Hutang</div>
          <div>
          ...
          <table class="table table-sm table-hover table-bordered align-middle mb-0" style="font-size: 0.85rem;">
            <thead class="table-secondary text-center align-middle sticky-top">
              <tr>
                ...
                <th width="25%" class="py-2">Pihak Hutang (Kreditur)</th>
                <th width="17%" class="py-2">Saldo Awal</th>
                <th width="17%" class="py-2 text-danger">Penambahan Hutang (+)</th>
                <th width="17%" class="py-2 text-success">Pelunasan Hutang (-)</th>
            ...
            <tbody>
              ...
              <tr v-for="(item, index) in filteredReportData" :key="item.id" :class="{'bg-light': index % 2 !== 0}">
                ...
                <td class="text-end fw-bold text-danger">Rp {{ formatNominal(item.tambah) }}</td>
                <td class="text-end fw-bold text-success">Rp {{ formatNominal(item.kurang) }}</td>
                <td class="text-end fw-bold text-primary bg-light bg-opacity-50">Rp {{ formatNominal(item.akhir) }}</td>
              </tr>
            </tbody>
            <tfoot v-if="!isLoading && filteredReportData.length > 0" class="table-dark fw-bold">
              <tr>
                ...
                <td class="text-end py-2 text-danger">Rp {{ formatNominal(totals.tambah) }}</td>
        ...
        <table class="w-100 table-print table-bordered border-dark text-dark mb-4">
          <thead class="text-center fw-bold bg-light" style="font-size: 9pt;">
            <tr>
              ...
              <th width="25%" class="p-1">Pihak Hutang (Kreditur)</th>
              <th width="17%" class="p-1">Saldo Awal</th>
              <th width="17%" class="p-1">Penambahan (+)</th>
...
const fetchData = async () => {
  ...
  AppAlert.loading('Mengumpulkan data hutang...')

  try {
    const { data: masterData, error: errMaster } = await supabase.from('master_hutang').select('id, pihak_lawan(nama)')
    if (errMaster) throw errMaster
    const masterMap = new Map()
    ;(masterData || []).forEach((m: any) => masterMap.set(m.id, m.pihak_lawan?.nama || 'Unknown'))

    const { data: tagihanData, error: errTagihan } = await supabase.from('tagihan_hutang').select('master_hutang_id, jumlah_tagihan, tanggal_tagihan').lte('tanggal_tagihan', filters.value.endDate)
    if (errTagihan) throw errTagihan

    const { data: bayarData, error: errBayar } = await supabase.from('pembayaran_hutang').select('jumlah_bayar, tanggal_pembayaran, tagihan_hutang(master_hutang_id)').lte('tanggal_pembayaran', filters.value.endDate)
    ...
    ;(tagihanData || []).forEach((t: any) => {
      if (!t.master_hutang_id) return
      const row = getRow(t.master_hutang_id)
    ...
    ;(bayarData || []).forEach((p: any) => {
      const masterId = p.tagihan_hutang?.master_hutang_id
      if (!masterId) return
...
const exportToExcel = () => {
  let tsvContent = `REKAPITULASI HUTANG\nPeriode: ${filters.value.startDate} s/d ${filters.value.endDate}\n\n`
  tsvContent += "No\tPihak Hutang (Kreditur)\tSaldo Awal\tPenambahan (+)\tPelunasan (-)\tSaldo Akhir\n"
  ...
  link.setAttribute("download", `Rekap_Hutang_${filters.value.startDate}.xls`)
