<template>
  <div class="master-hutang">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        <h4 class="fw-bold text-dark mb-0">Master Hutang (Perjanjian)</h4>
        <p class="text-muted small mb-0">Wadah data kontrak/perjanjian hutang sebelum rincian tagihan</p>
      </div>
          ...
          <thead class="table-dark text-center align-middle">
            <tr>
              ...
              <th width="16%" class="py-3 cursor-pointer" @click="handleSort('pihak_lawan_nama')">
                Pihak Lawan <i class="bi ms-1" :class="sortColumn==='pihak_lawan_nama' ? (sortDirection==='asc' ? 'bi-sort-alpha-down' : 'bi-sort-alpha-up') : 'bi-arrow-down-up opacity-25'"></i>
              </th>
              ...
              <th width="15%" class="py-3 cursor-pointer" @click="handleSort('sisa_tagihan')">
                Sisa Tagihan <i class="bi ms-1" :class="sortColumn==='sisa_tagihan' ? (sortDirection==='asc' ? 'bi-sort-numeric-down' : 'bi-sort-numeric-up') : 'bi-arrow-down-up opacity-25'"></i>
              </th>
        ...
        <div class="row g-0">
          ...
          <div :class="(isEditMode || isReadOnlyMode) ? 'col-md-7 border-end' : 'col-12'">
            <div class="card-body p-4 max-h-600 overflow-auto custom-scrollbar">
              ...
              <div class="row g-3">
                <div class="col-12">
                  <label class="form-label small fw-bold">Pihak Lawan (Vendor/Mitra) <span v-if="!isReadOnlyMode" class="text-danger">*</span></label>
                  ...
                  <small v-if="isEditMode" class="text-muted" style="font-size: 0.7rem;">Pihak lawan tidak dapat diubah setelah perjanjian dibuat.</small>
                </div>
              </div>
              ...
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <input type="text" class="form-control rounded-0" v-model="form.nomor_perjanjian" placeholder="Contoh: SPK/2026/001" :disabled="isReadOnlyMode">
                </div>
                <div class="col-md-6">
                ...
                <div class="col-12">
                  ...
                  <textarea class="form-control rounded-0" v-model="form.keterangan" rows="3" placeholder="Deskripsi singkat mengenai hutang ini..." :disabled="isReadOnlyMode"></textarea>
                </div>
              </div>
            </div>
...
const filteredData = computed(() => {
  if (!searchQuery.value) return masterHutangs.value
  const q = searchQuery.value.toLowerCase()
  return masterHutangs.value.filter(item => {
...
const form = ref<any>({
  id: '', pihak_lawan_id: '', nomor_perjanjian: '', tanggal_mulai: '', keterangan: ''
})
...
const fetchDropdowns = async () => {
  ...
  } catch (err) { console.error('Gagal memuat Pihak Lawan', err) }
}

const fetchData = async () => {
  AppAlert.loading('Memuat data...')
  try {
    const { data, error } = await supabase.from('v_master_hutang').select('*').order('tanggal_mulai', { ascending: false })
    if (error) throw error
    masterHutangs.value = data || []
    AppAlert.close()
...
const loadHistory = async (id: string) => {
  try {
    const { data } = await supabase.from('master_hutang_history').select('*').eq('master_hutang_id', id).order('waktu_ubah', { ascending: false })
    histories.value = data || []
  } catch (err) { console.error('Gagal tarik histori', err) }
}

const openAddModal = () => {
  if (!canCreate.value) { AppAlert.error('Akses Ditolak', 'Anda tidak berhak menambah data.'); return }
...
const saveData = async () => {
  ...
  if (isEditMode.value && !canUpdate.value) return
  ...
  try {
    ...
    if (!isEditMode.value) {
      const { error } = await supabase.from('master_hutang').insert([{ ...payload, created_by: currentUsername, created_at: currentTime }])
      if (error) throw error
      AppAlert.success('Berhasil', 'Master Hutang baru berhasil dibuat.')
    } 
    else {
      const { error } = await supabase.from('master_hutang').update(payload).eq('id', form.value.id)
      if (error) throw error
      ...
      for (let field of fieldsToTrack) {
        if (String(form.value[field] || '') !== String(originalFormData[field] || '')) {
          changes.push({
            master_hutang_id: form.value.id, kolom_diubah: field,
            nilai_lama: String(originalFormData[field] || ''), nilai_baru: String(form.value[field] || ''),
            diubah_oleh: currentUsername
          })
        }
      }
      if (changes.length > 0) await supabase.from('master_hutang_history').insert(changes)
...
const deleteData = async (id: string, no_perjanjian: string) => {
  ...
  const result = await Swal.fire({
    title: 'Hapus Data?', 
    html: `Hapus Master Hutang <b>${no_perjanjian}</b>?<br><small class="text-danger">Seluruh tagihan dan pembayaran yang terkait juga akan ikut terhapus.</small>`,
    icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'Ya, Hapus!'
  })
  if (result.isConfirmed) {
    AppAlert.loading('Menghapus...')
    try {
      const { error } = await supabase.from('master_hutang').delete().eq('id', id)
      if (error) throw error
...
.custom-modal-card { width: 95%; max-height: 90vh; animation: slideDown 0.3s ease-out; transition: max-width 0.3s ease; }
