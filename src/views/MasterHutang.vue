        ...
        <table class="table table-striped table-hover table-bordered align-middle mb-0" style="min-width: 1200px;">
          <thead class="table-dark text-center align-middle">
            <tr>
              ...
              <th width="16%" class="py-3">Pihak Lawan</th>
              ...
              <th width="15%" class="py-3">Sisa Tagihan</th>
              <th width="12%" class="py-3">Aksi</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="masterHutangs.length === 0">
              <td colspan="7" class="text-center text-muted py-5">
                <i class="bi bi-file-earmark-text fs-2 d-block mb-2"></i> Belum ada data perjanjian hutang.
              </td>
            </tr>
            
            <tr v-for="(item, index) in masterHutangs" :key="item.id">
        ...
        <div class="row g-0">
          ...
          <div :class="(isEditMode || isReadOnlyMode) ? 'col-md-7 border-end' : 'col-12'">
            <div class="card-body p-4 max-h-600 overflow-auto custom-scrollbar">
              ...
              <!-- Kelompok 2: Detail Kontrak -->
              <div class="row g-3">
                <div class="col-md-6">
                  <label class="form-label small fw-bold">Nomor Perjanjian <span v-if="!isReadOnlyMode" class="text-danger">*</span></label>
                  <input type="text" class="form-control" v-model="form.nomor_perjanjian" placeholder="Contoh: SPK/2026/001" :disabled="isReadOnlyMode">
                </div>
                ...
                <div class="col-12">
                  ...
                  <textarea class="form-control" v-model="form.keterangan" rows="3" placeholder="Deskripsi singkat mengenai hutang ini..." :disabled="isReadOnlyMode"></textarea>
                </div>
              </div>
...
// Menghapus total_nilai dari state form
const form = ref<any>({
...
const fetchData = async () => {
  ...
  try {
    // MENGAMBIL DATA DARI VIEW (v_master_hutang) AGAR TOTAL KALKULASI MUNCUL
    const { data, error } = await supabase
      .from('v_master_hutang')
      .select('*')
...
const loadHistory = async (id: string) => {
  try {
    const { data } = await supabase
      .from('master_hutang_history')
      .select('*')
      .eq('master_hutang_id', id)
...
const openAddModal = () => {
  if (!canCreate.value) {
    AppAlert.error('Akses Ditolak', 'Anda tidak berhak menambah data.')
    return
  }
...
const saveData = async () => {
  ...
  try {
    ...
    // Payload disimpan ke tabel asli (master_hutang)
    const payload = {
      pihak_lawan_id: form.value.pihak_lawan_id,
      nomor_perjanjian: form.value.nomor_perjanjian,
    ...
    if (!isEditMode.value) {
      const { error } = await supabase.from('master_hutang').insert([{
        ...payload, created_by: currentUsername, created_at: currentTime
      }])
      if (error) throw error
      ...
      for (let field of fieldsToTrack) {
        if (String(form.value[field] || '') !== String(originalFormData[field] || '')) {
          changes.push({
            master_hutang_id: form.value.id,
            kolom_diubah: field,
      ...
      if (changes.length > 0) {
        await supabase.from('master_hutang_history').insert(changes)
      }
...
const deleteData = async (id: string, no_perjanjian: string) => {
  if (!canDelete.value) {
    AppAlert.error('Akses Ditolak', 'Anda tidak berhak menghapus data.')
    return
  }
  ...
  if (result.isConfirmed) {
    ...
    try {
      ...
      AppAlert.error('Gagal menghapus', err.message || err) 
    }
  }
}
</script>
